...
Main = (function() {
  function Main() {
    this.ORIGIN_URL = "http://gonshi.github.io/banrigan/";
    this.latLngAsset = require("../../json/latlng.json");
    this.social_txt = document.querySelector(".social_txt");
    ...
    this.social_btn = document.querySelector(".social_btn");
    this.link_balloon = document.querySelector(".link_balloon");
    this.exec();
  }

  Main.prototype.popup = function(url) {
    var _dualScreenLeft, _dualScreenTop, _left, _popupHeight, _popupWidth, _top, _windowHeight, _windowWidth, ref;
    ...
    } else if (((ref = document.documentElement) != null ? ref.clientWidth : void 0) != null) {
    ...
    return window.open(url, "popup", ("width=" + _popupWidth + ", height=" + _popupHeight + ", ") + ("top=" + _top + ", left=" + _left));
  };

  Main.prototype.getShareUrl = function() {
    var _url;
    _url = (this.ORIGIN_URL + "?lat=" + (this.map.getCenter().lat())) + ("&lng=" + (this.map.getCenter().lng()) + "&heading=" + (Math.floor(this.sv.pov.heading)));
    return _url;
  };

  Main.prototype.setSocial = function() {
    var _fb_appId, fjs, js;
    ...
    this.social_btns.facebook.addEventListener("click", (function(_this) {
      return function() {
        return FB.ui({
          method: "share",
    ...
    this.social_btns.tweet.addEventListener("click", (function(_this) {
      return function() {
        _this.social_txt.setAttribute("value", _this.getShareUrl());
        return _this.popup(("http://twitter.com/share?url=" + (encodeURIComponent(_this.getShareUrl())) + "&text=") + ((encodeURIComponent("万里眼は、Chromeで新規タブを開く度に地球上のランダムな" + "位置のストリートビューを観ることができるChrome拡張です。")) + "&hashtags=万里眼"));
      };
    })(this));
    this.social_btns.hatena.addEventListener("click", (function(_this) {
      return function() {
        return _this.popup(("http://b.hatena.ne.jp/add?url=" + (encodeURIComponent(_this.getShareUrl())) + "&") + ("title=" + (encodeURIComponent("万里眼"))));
      };
    })(this));
    return this.social_btns.gplus.addEventListener("click", (function(_this) {
      return function() {
        return _this.popup("https://plus.google.com/share?url=" + (encodeURIComponent(_this.getShareUrl())));
      };
    })(this));
  };

  Main.prototype.render = function(latLng, heading) {
    var _heading, _rotateControl;
    _heading = parseInt(heading) || 165;
    _rotateControl = navigator.userAgent.match(/iphone|ipod|ipad|android/i) ? false : true;
    this.sv = new google.maps.StreetViewPanorama(document.getElementById("sv"), {
      addressControl: !this.hide_banner,
      linksControl: !this.hide_banner,
      zoomControl: !this.hide_banner,
      panControl: !this.hide_banner,
      scrollwheel: !this.hide_banner,
      rotateControl: _rotateControl,
      fullscreenControl: !this.hide_banner,
      position: latLng,
      pov: {
        heading: _heading,
        pitch: 0
    ...
    if (!this.hide_banner) {
      ...
      return this.social_btn.style.display = "block";
    }
  };

  Main.prototype.getPlace = function() {
    var _rand;
    _rand = Math.floor(Math.random() * this.latLngAsset.length);
    return this.render(new window.google.maps.LatLng(this.latLngAsset[_rand].lat, this.latLngAsset[_rand].lng));
  };

  Main.prototype.searchPlace = function() {
    var _location;
    this.sv = new google.maps.StreetViewService();
    _location = new window.google.maps.LatLng((Math.random() - 0.5) * 120, (Math.random() - 0.5) * 360);
    return this.sv.getPanoramaByLocation(_location, 400000, (function(_this) {
      return function(data) {
        if (data) {
          return _this.render(data.location.latLng);
        } else {
          return _this.getPlace();
        }
      };
    })(this));
  };
  ...
  Main.prototype.exec = function() {
    var _lang;
    window.initMap = (function(_this) {
      return function() {
        var _heading;
        if (location.search.match("lat")) {
          _heading = location.search.match(/(\&|＆)heading=(.*?)(\&|$)/) ? location.search.match(/(\&|＆)heading=(.*?)(\&|$)/)[2] : null;
          return _this.render(new window.google.maps.LatLng(location.search.match(/\?lat=(.*?)\&/)[1], location.search.match(/(\&|＆)lng=(.*?)(\&|$)/)[2]), _heading);
        } else {
          return _this.searchPlace();
        }
      };
    })(this);
    this.social_btn.addEventListener("click", (function(_this) {
      return function() {
        _this.social_txt.setAttribute("value", _this.getShareUrl());
        if (_this.social.getAttribute("class").match("is_show")) {
          return _this.social.classList.remove("is_show");
        } else {
          _this.social.classList.add("is_show");
          return _this.social_txt.select();
        }
      };
    })(this));
    this.social_btns.link.addEventListener("click", (function(_this) {
      return function() {
        var _range, err;
        if (_this.social_btns.link.getAttribute("class").match("is_active")) {
          _this.social_btns.link.classList.remove("is_active");
        } else {
          _this.social_btns.link.classList.add("is_active");
          _this.social_btns.link.innerHTML = _this.getShareUrl();
          _range = document.createRange();
          _range.selectNode(_this.social_btns.link);
          window.getSelection().addRange(_range);
          try {
            document.execCommand("copy");
            _this.link_balloon.classList.add("is-show");
            clearTimeout(_this.balloon_timer);
            _this.balloon_timer = setTimeout((function() {
              return _this.link_balloon.classList.remove("is-show");
            }), 3000);
          } catch (error) {
            err = error;
            console.log(err);
    ...
    this.hide_banner = location.search.match("hide_banner") ? true : false;
    _lang = window.navigator.userLanguage || window.navigator.language || window.navigator.browserLanguage;
    this.lang = _lang.match("ja") ? "ja" : "en";
    if (this.lang === "en") {
      document.body.classList.add("is_en");
    }
    this.setSocial();
    if (window === parent) {
      return document.querySelector(".toStore").style.display = "block";
    }
  };
